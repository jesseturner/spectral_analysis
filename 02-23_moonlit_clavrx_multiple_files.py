...
def main():
    ...
    da_cloud_mask, datetime_str = get_cloud_mask_multiple(orbit_files)
    plot_clavrx_cloud_mask(da_cloud_mask, datetime_str, save_path="clavrx_cloud_mask_multiple")
...
def get_cloud_mask_multiple(orbit_files):
    orbit_files_sample = orbit_files[0:7]
    print("Plotting a selection from the orbit...")
    print("Runs into problems with some files (HDF error), not sure why yet.")
    ...
    start_dt = extract_datetime_from_filename(orbit_files_sample[0])
    end_dt   = extract_datetime_from_filename(orbit_files_sample[-1])
    if start_dt and end_dt:
        datetime_str = f"{start_dt} - {end_dt}"
    else:
        datetime_str = "Unknown - File format unexpected."

    da_cloud_mask = ds_combined['cloud_mask']
    print(da_cloud_mask)
    valid = np.isfinite(da_cloud_mask) & np.isfinite(da_cloud_mask["latitude"]) & np.isfinite(da_cloud_mask["longitude"])
    da_cloud_mask = da_cloud_mask.where(valid.compute(), drop=True)
