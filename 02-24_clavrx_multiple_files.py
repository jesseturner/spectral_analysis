...
def main():
    ...
    # da_cloud_mask, datetime_str = get_cloud_mask_single(orbit_files[sample_id])
    # plot_clavrx_cloud_mask(da_cloud_mask, datetime_str, save_path="clavrx_cloud_mask_file")

    sample_start, sample_end = 0, 12
    ...
    da_cloud_mask, datetime_str = get_cloud_mask_multiple(orbit_files_select)
...
def get_cloud_mask_single(file):
    ds_example = xr.open_dataset(
        file,
        engine='netcdf4')

    match = re.search(r'd(\d{8})_t(\d{6})', file)
    if match:
